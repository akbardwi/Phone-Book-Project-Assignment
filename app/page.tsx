...
const Page = () => {
    ...
    const getContacts = async () => {
        ...
        if (data.contact) {
            const listContact:any = [...data.contact].sort((a:any, b:any) => b.id - a.id);
    ...
    const handleFavorite = async (id: any) => {
        try {
            const contactToFavorite = contacts.filter((cnt) => cnt.id == id);
            if(contactToFavorite){
                const updatedFavorite:any = [
                    ...favoriteContacts,
                    contactToFavorite[0]
                ].sort((a:any, b:any) => b.id - a.id);
                
                setFavoriteContacts(updatedFavorite);

                localStorage.setItem("favoriteContacts", JSON.stringify(updatedFavorite));
            }
        } catch (e: any) {
            toast.error(e.message);
        }

        const updateContacts = contacts.filter((cnt) => cnt.id != id);
        setContacts(updateContacts);
        toast.success("Contact added to favorite successfully!");
    }

    const handleUnFavorite = async (id: any) => {
        const contactToUnFavorite = favoriteContacts.filter((cnt:any) => cnt.id == id);
        try {
            if(contactToUnFavorite){
                const updatedFavorite:any = favoriteContacts.filter((cnt:any) => cnt.id != id);
    ...
    useEffect(() => {
        // Update the filtered contacts when the contacts change
    ...
    useEffect(() => {
        ...
        const listContacts = contacts.filter((contact) =>
            !favoriteContacts.some((favContact:any) => favContact.id === contact.id)
        );
                ...
                isLoggedIn && !isLoading ? (
                    <div className="">
                        ...
                        <div className="mt-10 m-5 border border-2 border-gray-200 py-4 rounded h-[400px] overflow-y-auto">
                            ...
                            {
                                favoriteContacts && favoriteContacts.length > 0 ? favoriteContacts.map((favContact:any) => {
                                    return(!isContactsLoading && (
                                        <div className="relative m-4 bg-gray-800 text-white px-3 py-2 rounded" key={favContact.id}>
                                            ...
                                            <div className="my-3">
                                                <h2 className="text-2xl font-bold">
                                                    {
                                                        favContact.first_name.slice(0, 1).toUpperCase()
                                                    }
                                                    {
                                                        favContact.first_name.slice(1, favContact.first_name.length) + " "
                                                    } 
                                                    {
                                                        favContact.last_name.slice(0, 1).toUpperCase()
                                                    }
                                                    {
                                                        favContact.last_name.slice(1, favContact.last_name.length)
                                                    } 
                                                </h2>
                                                <div className="mt-2 mb-4">
                                                    {favContact.phones.length > 0 ? favContact.phones.map((num:any) => {
                                                        return (
                                            ...
                                            <div className="my-2 flex justify-evenly">
                                                {favContact.phones.length > 0 && (
                                                    <p className="text-md">
                                                    <Icon onClick={
                                                            () => copyNumber(favContact.phones[0].number)
                                                        }
                                                ...
                                                <p className="text-md">
                                                    <Icon onClick={
                                                            () => editContact(favContact.id)
                                                        }
                                                        icon="material-symbols:edit-square-outline-rounded"/>
                                                </p>
                                                <p className="text-md">
                                                    <Icon onClick={
                                                            () => deleteContact(favContact.id)
                                                        }
                                                        icon="material-symbols:delete-outline-rounded"/>
                                                </p>
                                                <p className="text-md">
                                                    <Icon onClick={
                                                            () => handleUnFavorite(favContact.id)
                                                        }
                                                        icon="mdi:star"/>
                                                </p>
                                            </div>
